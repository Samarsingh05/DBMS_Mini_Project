...
def search():
    ...
    def searchBook():
        ...
        try:
            ...
            cur.callproc('GetBookDetailsById', (bookid))

            # Fetch the results returned by the stored procedure
            for result in cur.fetchall():
                Label(labelFrame, text="%-10s%-35s%-25s%-15s" % (result[0], result[1], result[2], result[3]), bg='#222831', fg='white', font=('Consolas', 10)).place(relx=0.07, rely=y)
                y += 0.1
